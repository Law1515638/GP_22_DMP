...
/**
  * Description: 需求指标四  统计各操作系统 请求及竞标参数
  *
  * @author Law
...
//    sparkCore(df, sQLContext)
  ...
  def sparkSQL(df: DataFrame, sQLContext: SQLContext): Unit = {
    ...
    val RptDF: DataFrame = sQLContext.sql("select " +
      ...
      "( " +
      "select " +
      "(case client when 1 then 'android' when 2 then 'ios' when 3 then 'wp' else '其他' end) client, " +
  ...
  def sparkCore(df: DataFrame, sQLContext: SQLContext): Unit = {
    // 将数据进行处理，统计各种指标
    val operatingSystemTup: RDD[(String, List[Double])] = df.map(row => {
      // 把需要的字段全部取到
      ...
      if (clientType == 2) client = "ios"
      if (clientType == 3) client = "wp"
      (client, list)
    })
    // 聚合
    val RptRDD: RDD[(String, List[Double])] = operatingSystemTup.reduceByKey((x, y) => (x zip y).map(x => x._1 + x._2))

    val RptRow: RDD[Row] = RptRDD.map(rdd => {
      val list = rdd._2
      Row(rdd._1, list(0), list(1), list(2), list(3), list(4), list(5), list(6), list(7), list(8))
    })
    val schema = StructType(StructField("操作系统", StringType) ::
