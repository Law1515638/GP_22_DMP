...
/**
  * Description: 需求指标四  统计各登陆设备 请求及竞标参数
  *
  * @author Law
...
//        sparkSQL(df, sQLContext)
  ...
  def sparkSQL(df: DataFrame, sQLContext: SQLContext): Unit = {
    ...
    val RptDF: DataFrame = sQLContext.sql("select " +
      ...
      "( " +
      "select " +
      "(case devicetype when 1 then '手机' when 2 then '平板' else '其他' end) device, " +
  ...
  def sparkCore(df: DataFrame, sQLContext: SQLContext): Unit = {
    // 将数据进行处理，统计各种指标
    val deviceTup: RDD[(String, List[Double])] = df.map(row => {
      // 把需要的字段全部取到
      ...
      if (devicetype == 2) device = "平板"
      (device, list)
    })
    // 聚合
    val RptRDD: RDD[(String, List[Double])] = deviceTup.reduceByKey((x, y) => (x zip y).map(x => x._1 + x._2))

    val RptRow: RDD[Row] = RptRDD.map(rdd => {
      val list = rdd._2
      Row(rdd._1, list(0), list(1), list(2), list(3), list(4), list(5), list(6), list(7), list(8))
    })
    val schema = StructType(StructField("设备类型", StringType) ::
