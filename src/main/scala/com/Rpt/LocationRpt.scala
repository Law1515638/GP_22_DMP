...
/**
  * Description: 需求指标三  统计各地域 请求及竞标参数
  *
  * @author Law
...
//    sparkSQL(df, sQLContext)
  ...
  def sparkCore(df: DataFrame, sQLContext: SQLContext): Unit = {
    // 将数据进行处理，统计各种指标
    val proAndCityTup: RDD[((String, String), List[Double])] = df.map(row => {
    // 把需要的字段全部取到
    ...
    // 聚合
    val RptRDD: RDD[((String, String), List[Double])] = proAndCityTup.reduceByKey((x, y) => (x zip y).map(x => x._1 + x._2))
